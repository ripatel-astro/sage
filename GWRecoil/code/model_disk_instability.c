...
void check_disk_instability(int p, int centralgal, int halonr, double time,
  ...
  if (is_greater(diskmass, 0.0)) {
    ...
    if (is_greater(unstable_stars, 0.0)) {
      ...
      Gal[p].BulgeMass += unstable_stars; // why doesn't this get subtracted from the cold gas or stellar mass variable? 
    ...
    if (is_greater(unstable_gas, 0.0)) {
      ...
      if (SageConfig.AGNrecipeOn > 0 && Gal[p].BHejectFlag == 0) {
        grow_black_hole(p, unstable_gas_fraction);
      }
      else if (SageConfig.AGNrecipeOn > 0 && Gal[p].BHejectFlag == 1) {
        Gal[p].BulgeMass += unstable_gas; // If AGN feedback is disabled due to ejection, transfer all unstable gas to bulge as stars (star formation efficiency???)
      }
