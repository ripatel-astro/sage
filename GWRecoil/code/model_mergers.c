...
void deal_with_galaxy_merger(int p, int merger_centralgal, int centralgal,
   ...
   * & Haehnelt 2000) */
  if (SageConfig.AGNrecipeOn && Gal[merger_centralgal].BHejectFlag == 0) {
...
void add_galaxies_together(int t, int p) {
  ...
  /* Compute black hole mass ratios */
  Gal[t].BHMassRatio = calc_bh_mass_ratio(Gal[t].BlackHoleMass, Gal[p].BlackHoleMass);
  /* Add black hole recoil*/
  if (SageConfig.BHRecoilOn > 0) {
    float q = Gal[t].BHMassRatio;
    float a_1=0, a_2=0, e=0, xi=0, phi_1=0, phi_2=0, theta=0;
    ...
    if (SageConfig.BHRecoilOn == 1) {
      a_1=0, a_2=0, e=0, xi=0, phi_1=0, phi_2=0, theta=0;
    }
    else if (SageConfig.BHRecoilOn == 2) {
      e=0, xi=0, phi_1=0, phi_2=0, theta=0;
      Gal[t].PrimaryBHSpin = 0.998*gsl_rng_uniform(random_generator);
      Gal[t].SecondaryBHSpin = 0.998*gsl_rng_uniform(random_generator);
      a_1 = Gal[t].PrimaryBHSpin;
      a_2 = Gal[t].SecondaryBHSpin;
    }
    else if (SageConfig.BHRecoilOn == 3) {
      e=0, xi=0, phi_1=0, phi_2=0, theta=0;
      Gal[t].PrimaryBHSpin = -0.998*gsl_rng_uniform(random_generator);
      Gal[t].SecondaryBHSpin = -0.998*gsl_rng_uniform(random_generator);
      a_1 = Gal[t].PrimaryBHSpin;
    ...
    else if (SageConfig.BHRecoilOn == 4) {
      e=0, theta=0;
      Gal[t].PrimaryBHSpin = 0.998*gsl_rng_uniform(random_generator);
      Gal[t].SecondaryBHSpin = 0.998*gsl_rng_uniform(random_generator);
      a_1 = Gal[t].PrimaryBHSpin;
      a_2 = Gal[t].SecondaryBHSpin;
      xi = gsl_rng_uniform(random_generator)*2*M_PI;
      phi_1 = gsl_rng_uniform(random_generator)*M_PI;
      phi_2 = gsl_rng_uniform(random_generator)*M_PI;

    }    
    Gal[t].BHRecoilVmag = calc_recoil_velocity(q, e, a_1, a_2, xi, phi_1, phi_2, theta);

    /* Add black hole mass */
    Gal[t].BlackHoleMass += Gal[p].BlackHoleMass;
    Gal[t].BHmergeCount += Gal[p].BHmergeCount;

    /* Eject post-merger black hole if recoil velocity higher than escape velocity*/ 
    eject_black_hole(t, Gal[t].BHRecoilVmag); 
    Gal[t].BHEjectedMass += Gal[p].BHEjectedMass; 
    Gal[t].BHejectCount += Gal[p].BHejectCount;
