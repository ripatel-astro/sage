...
void init_galaxy(int p, int halonr) {
  ...
  Gal[p].SecondaryBHSpin = 0.0;
  Gal[p].BHMassRatio = 0.0;
...
/*CALCULATE BLACK HOLE MASS RATIO*/
double calc_bh_mass_ratio(double central_mass, double satellite_mass) {
  double primary_mass, secondary_mass;

  if (central_mass > 0 && satellite_mass > 0) {
    if (central_mass >= satellite_mass) {
      primary_mass = central_mass;
      secondary_mass = satellite_mass;
    }
    else {
      primary_mass = satellite_mass;
      secondary_mass = central_mass;
    }
...
/*DEFINES BH GW RECOIL VELOCITY FUNCTION*/
double calc_recoil_velocity(float q, float e, float a_1, float a_2, float xi, float phi_1, float phi_2, float theta){
  
  double p, a_1_perp, a_2_perp, a_1_par, a_2_par, v_m, v_perp, v_par, v_rec;
  double A, B, H, K;
  ...
  p = pow(q, 2)/pow((1+q),5);
  a_1_perp = a_1*sin(phi_1);
  a_2_perp = a_2*sin(phi_2);
  a_1_par = a_1*cos(phi_1);
  a_2_par = a_2*cos(phi_2);
  v_m = (A*p*(1-q))*(1+(B*(q/pow((1+q),2)))); //velocity based on unequal mass*/
  v_perp =  (H*p)*(a_1_par - q*a_2_par); //velocity perpendicular to orbital angular momentum direction
  v_par = (K*p*cos(theta))*(a_1_perp - q*a_2_perp); //velocity parallel to orbital angular momentum direction

  v_rec = (1+e)*sqrt(pow(v_m,2)+pow(v_perp,2)+pow(v_par,2)+2*v_m*v_perp*cos(xi));
    
  return v_rec;
}

/* EJECT BLACK HOLE IF RECOIL VELOCITY EXCEEDS ESCAPE VELOCITY */
void eject_black_hole(int gal_index, double recoil_velocity) {
  float Vesc = sqrt(2)*Gal[gal_index].Vvir;
  if (recoil_velocity >= Vesc && recoil_velocity > 0) {
    Gal[gal_index].BHEjectedMass += Gal[gal_index].BlackHoleMass; //this places the ejected black hole into a reservoir, which we can reincorporate later
    Gal[gal_index].MassofLastEjectedBH = Gal[gal_index].BlackHoleMass; //this tracks the mass of the last ejected black hole, which can be used for diagnostics and/or reincorporation
    Gal[gal_index].BlackHoleMass = 0; //this removes the ejected black hole from the galaxy
    Gal[gal_index].PrimaryBHMass = 0;
    Gal[gal_index].SecondaryBHMass = 0;
...
/*ASSIGN BLACK HOLE MERGER TYPE*/
void assign_bh_merger_type(int centralgal, int satellitegal) {
  if (Gal[centralgal].BHejectFlag == 0) {
    if (Gal[centralgal].BlackHoleMass > 0 && Gal[satellitegal].BlackHoleMass > 0) {
      Gal[centralgal].BHmergeType = 1;
      Gal[centralgal].BHmergeCount++; 
    }
    else {
      Gal[centralgal].BHmergeType = 0; // no BH merger, because one or both galaxies have no black hole
    }
  }
  if (Gal[centralgal].BHejectFlag == 1) {
    if (Gal[centralgal].BlackHoleMass == 0 && Gal[satellitegal].BlackHoleMass > 0) {
...
double form_nsc(float StellarMass){  
  ...
  if (logMstar <= 9 + (.38/.44)){        //small galaxies
    ...
    logMnsc_noscatter = (.92*(logMstar-9) + 6.13);
  }
  ...
  if (random_generator == NULL) {
    ...
    gsl_rng_set(random_generator, time(NULL));
  }
  
  logMnsc = logMnsc_noscatter + gsl_ran_gaussian(random_generator, SageConfig.NSCMassScatter);
  
  Mnsc = pow(10, logMnsc)/1e10;
  return Mnsc;
}
