...
def plot(
    ...
    w = np.where((galaxies.BlackHoleMass > 1e-10))[0] 
    ...
    PrimaryBHMass = galaxies.PrimaryBHMass[w]
    PrimaryBHSpin = galaxies.PrimaryBHSpin[w]
    SecondaryBHMass = galaxies.SecondaryBHMass[w]
    SecondaryBHSpin = galaxies.SecondaryBHSpin[w]
    ...
    if verbose:      
        ...
        print(f"  Primary Black Hole Masses: {min(PrimaryBHMass):.2f} to {max(PrimaryBHMass):.2f}")
        print(f"  Primary Black Hole Spins: {min(PrimaryBHSpin):.2f} to {max(PrimaryBHSpin):.2f}")
        print(f"  Secondary Black Hole Masses: {min(SecondaryBHMass):.2f} to {max(SecondaryBHMass):.2f}")
        print(f"  Secondary Black Hole Spins: {min(SecondaryBHSpin):.2f} to {max(SecondaryBHSpin):.2f}")
    #print(BTRatio)
    ...
    # Customize the plot
    ax.set_xlabel(r"Black Hole Spin", fontsize=AXIS_LABEL_SIZE)
    ax.set_ylabel(r"Black Hole Count", fontsize=AXIS_LABEL_SIZE)
    ax.set_title(r"Black Hole Spin Distribution", fontsize=AXIS_LABEL_SIZE)
