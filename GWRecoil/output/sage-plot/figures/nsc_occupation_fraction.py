...
"""
SAGE Nuclear Star Clusters occupation fraction

This module generates a plot showing the relationship between galaxy stellar mass vs. nuclear star cluster occupation fraction for SAGE galaxy data.
"""
...
def plot(
    ...
    """
    Create a galaxy stellar mass vs nuclear star cluster occupation fraction chart.
    ...
    has_nsc = galaxies.NSC[w] > 0
     

    # Check if we have any galaxies to plot
    if len(w) == 0:
        print("No suitable galaxies found for nuclear star cluster occupation plot")
        # Create an empty plot with a message
        ax.text(
            0.5,
            0.5,
            "No suitable galaxies found for nuclear star cluster occupation plot",
        ...
        output_path = os.path.join(output_dir, f"NSCOccupation{output_format}")
    ...
    EarlyNSCGalaxy = logStellarMass[(SSFR < 10**SSFRThreshold) & (has_nsc)]
    LateGalaxy = logStellarMass[SSFR >= 10**SSFRThreshold]
    LateNSCGalaxy = logStellarMass[(SSFR >= 10**SSFRThreshold) & (has_nsc)]

    bins = np.arange(0, 15, 1)
    EarlyGalaxies_count, edges = np.histogram(EarlyGalaxy, bins = bins)
    EarlyNSCGalaxies_count, _ = np.histogram(EarlyNSCGalaxy, bins = edges)
    LateGalaxies_count, edges_late = np.histogram(LateGalaxy, bins = edges)
    LateNSCGalaxies_count, _ = np.histogram(LateNSCGalaxy, bins = edges)
    ...
    f_occ_early[e] = EarlyNSCGalaxies_count[e] / EarlyGalaxies_count[e]
    f_occ_late[l] =  LateNSCGalaxies_count[l] / LateGalaxies_count[l]
    ...
    ax.set_ylabel(r"Fraction of Galaxies with NSCs", fontsize=AXIS_LABEL_SIZE)
    ...
    output_path = os.path.join(output_dir, f"NSCOccupationPlot{output_format}")
    if verbose:
        print(f"Saving NSC Occupation plot to: {output_path}")
    plt.savefig(output_path)
    plt.close()
