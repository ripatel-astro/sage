...
def plot(
    ...
    if len(w) == 0:
        ...
        output_path = os.path.join(output_dir, f"BHRecoilDistribution_events{output_format}")
    ...
    ax.set_title(r"Black Hole Recoil Distribution (Events)", fontsize=AXIS_LABEL_SIZE)
    ...
    output_path = os.path.join(output_dir, f"BHRecoilDistribution_events{output_format}")
    if verbose:
        print(f"Saving Black Hole Recoil Distribution (events) plot to: {output_path}")
    plt.savefig(output_path)
    plt.close()
