...
def plot(
    ...
    # Define safe division function for usage in fraction calculations to avoid division by zero
    ...
    for i in range(len(sfr)):
        sSFR.append(safe_div(sfr[i], stellar_mass[i]))
    sSFR = np.array(sSFR)

    quiescent_mask = (sSFR < 10.0**sSFRcut)
    stellar_mass_quiescent = stellar_mass[quiescent_mask]
    ...
    mass = []  # Bin centers
    quiescent_fraction = []  # All galaxies
    ...
    for i in range(nbins - 1):
        ...
        quiescent_bin_mask = (stellar_mass_quiescent >= mass_bins[i]) & (stellar_mass_quiescent < mass_bins[i + 1])
        quiescent_count = np.sum(quiescent_bin_mask)

        # Calculate fraction
        quiescent_frac = safe_div(quiescent_count, this_bin_count)
        quiescent_fraction.append(quiescent_frac)
    ...
    mass = np.array(mass)
    quiescent_fraction = np.array(quiescent_fraction)


    if params["BHRecoilOn"] == 0:
        np.save('starfraction_norecoil.npy', quiescent_fraction)
    
   
    if params["BHRecoilOn"] == 1 or params["BHRecoilOn"] == 2 or params["BHRecoilOn"] == 3 or params["BHRecoilOn"] == 4:
        quiescent_frac_norecoil = np.load('starfraction_norecoil.npy')

        quiescent_frac_diff = []
        for i in range(len(mass)):
            frac_diff = safe_div(quiescent_fraction[i] - quiescent_frac_norecoil[i], quiescent_frac_norecoil[i])
            quiescent_frac_diff.append(frac_diff)
    ...
    if params['BHRecoilOn'] == 1 or params['BHRecoilOn'] == 2 or params['BHRecoilOn'] == 3 or params['BHRecoilOn'] == 4:
        ...
        ax.plot(
            ...
            quiescent_frac_diff,
            color='purple',
            label='Fractional Difference in Quiescent Fraction (with vs no recoil)',
        )



    # Customize the plot
    ax.set_title("Fraction of Quiescent Galaxies", fontsize=AXIS_LABEL_SIZE + 2)
...
#     if params["BHRecoilOn"] == 0:
#         np.save('starfraction_norecoil.npy', fraction)
    
#     if params["BHRecoilOn"] == 1 or params["BHRecoilOn"] == 2 or params["BHRecoilOn"] == 3 or params["BHRecoilOn"] == 4:
#         frac_norecoil = np.load('starfraction_norecoil.npy')
#         frac_diff = fraction - frac_norecoil
...
#     if np.any(mask):
#         if params["BHRecoilOn"] == 1 or params["BHRecoilOn"] == 2 or params["BHRecoilOn"] == 3 or params["BHRecoilOn"] == 4:
#             ax.plot(mass[mask], frac_norecoil[mask], color = 'black', label="All Galaxies (No Recoil)")
#             #ax.plot(mass[mask], frac_diff[mask], "k--", label="Difference from No Recoil", linewidth=0.8)
#         ax.plot(mass[mask], fraction[mask], color='cornflowerblue', linestyle = '--', label="All Galaxies", linewidth=2)
