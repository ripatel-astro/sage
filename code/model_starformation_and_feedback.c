...
void starformation_and_feedback(int p, int centralgal, double time, double dt,
  ...
  /*Calculate nuclear star cluster mass if enabled*/
  if (SageConfig.NuclearStarClustersOn == 1) {
    ...
    if (ICS > 0.0) {
      NSC_formed = fmin(NSC_target, ICS);
      Gal[p].ICS -= NSC_formed;
    }
    else if (ICS==0.0 && BulgeMass > 0.0) {
      NSC_formed = fmin(NSC_target, BulgeMass);
      Gal[p].BulgeMass -= NSC_formed;
      Gal[p].StellarMass -= NSC_formed;
